...
async function getIpfsImage(
  cid: string,
  {
    ...
    iv,
    encryptionKey,
  }: Pick<IpfsImageProps, "type" | "iv"> & { encryptionKey?: CryptoKey }
) {
  ...
  if (iv) {
    if (!encryptionKey) {
      throw new Error(
        "Can't load encrytped image from IPFS due to missing key"
      );
    }
    const encryptedBuffer = await imageBlob.arrayBuffer();
    const decyptedImageBuffer = await decrypt(
      encryptedBuffer,
      iv,
      encryptionKey
    );
...
const IpfsImage = (props: CommandExtensionProps<IpfsImageProps>) => {
...
): Promise<Partial<IpfsImageProps>> {
